...
umask 077
baseline=${RR_HEALTH_HOP_BASELINE_ROOT:-$fixture/official-721}
[ -n "${RR_HEALTH_HOP_BASELINE_ROOT:-}" ] || \
    python3 "$repo/tests/la721_baseline.py" "$baseline"

fail() { printf 'health hop observation: FAIL: %s\n' "$*" >&2; exit 1; }

# The full official 7.2.1 health module is the input, not a reconstructed loop.
# Keep its original hash and refusal behavior across later formal releases.
...
if [ "${RR_TEST_CURRENT_HEALTH:-0}" = 1 ]; then
    source "$repo/modules/10-system.sh"
    source "$repo/modules/30-singbox.sh"
...
# The shipped candidate must implement the same observation behavior. Run
# this test a second time with current production modules, while retaining
# the immutable historical module for the patch-identity/negative controls.
if [ "${RR_TEST_CURRENT_HEALTH:-0}" != 1 ]; then
    RR_TEST_CURRENT_HEALTH=1 bash "$0"
fi
