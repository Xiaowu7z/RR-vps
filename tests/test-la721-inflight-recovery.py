...
from la721_baseline import files as baseline_files
...
class RecoveryTests(unittest.TestCase):
    ...
    def test_marker_and_patch_match_official_baseline(self):
        ...
        baseline = os.environ.get("RR_HEALTH_HOP_BASELINE_ROOT")
        source = ((Path(baseline) / "modules/60-update.sh").read_bytes()
                  if baseline else baseline_files()["modules/60-update.sh"])
