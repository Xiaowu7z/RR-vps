...
个人订阅在 HTTPS 面板路由和主订阅 HTTP 服务两条链路上都会动态返回标准 `Subscription-Userinfo` 响应头（上传、下载、总额度、到期 Unix 时间），以及每小时更新建议。NekoBox、Clash 系等支持该标准的客户端可在订阅详情中显示已用、剩余流量和到期日期；不识别该响应头的客户端仍可正常订阅节点。
...
“服务器流量套餐”使用另一套独立计量：直接读取所选 Linux 公网网卡的 RX/TX 字节并把基线持久化到 SQLite，面板或进程重启后继续累计；网卡计数器回绕、VPS 重启或更换网卡时只重建基线，避免制造巨额假流量。可选择双向 `RX+TX`、仅服务器发送 `TX` 或仅服务器接收 `RX`，这比设备应用层流量更接近运营商账单，但不同服务商可能扣除链路层开销、内网流量或采用自己的换算单位，最终仍以服务商账单为准。服务器套餐达到上限只告警，不自动中断整台 VPS。
