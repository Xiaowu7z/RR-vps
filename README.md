...
> 当前版本：**7.2.4**（候选，尚未发布；验证方案待确认） · 当前正式版：**7.2.3** · [完整更新日志](CHANGELOG.md) · [GitHub Releases](https://github.com/Xiaowu7z/RR-vps/releases)

### 7.2.4 候选：Naive 首装与防火墙隔离恢复
...
原三台测试 VPS 已退役。[发布验证变更草案](docs/audit/release-v724-plan.md) 提议采用完整三发行版容器 CI，加此次单机恢复回执及候选文件的一致性核验；范围变化尚待用户确认，新门禁会拒绝未确认的方案。当前不声称三台实机或所有公网协议已验证。官方 7.2.1 与 Debian 兼容版旧更新入口对新产物格式的本地兼容回归已通过，主力服务器没有被修改。
...
`rr doctor`、加密 `.rrbak`、告警、TOTP/Passkey、历史图表、批量管理和 NaiveProxy HTTP/3 功能继续提供；`.rrbak` 与 NaiveProxy 的 7.2.0 恢复前置条件见下文，本版的安全边界、事务恢复、三发行版 CI 与三机实测证据见 Changelog。

## 一键安装

目标支持系统如下。CI 覆盖三种发行版容器，实际实机范围以当轮发布报告为准。7.2.3 及此前的三机门禁属于历史发布证据；7.2.4 的替代验证方案尚待确认，未完成的实机项目不得视为通过：

| 系统 | 支持度 | 说明 |
| --- | --- | --- |
| **Debian 12（bookworm）** | ⭐ 主推 | 容器 CI 目标；本次有单机定向恢复回执 |
| Ubuntu 22.04（jammy） | ✅ 目标支持 | 容器 CI 目标；本候选无新增实机回执 |
| Ubuntu 24.04（noble） | ✅ 目标支持 | 容器 CI 目标；本候选无新增实机回执 |
