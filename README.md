...
> 当前版本：**7.2.5** · [完整更新日志](CHANGELOG.md) · [GitHub Releases](https://github.com/Xiaowu7z/RR-vps/releases)
...
目标支持系统如下。CI 覆盖三种发行版容器，实际实机范围以当轮发布报告为准。7.2.3 及此前的三机门禁属于历史发布证据；7.2.4 起采用用户已批准的三发行版容器 CI 加恢复证据核验。7.2.5 保存原始历史回执，并独立核验新版本的下载修复与受限文件差异，未完成的实机项目不得视为通过：

| 系统 | 支持度 | 说明 |
| --- | --- | --- |
| **Debian 12（bookworm）** | ⭐ 主推 | 容器 CI 目标；保留此前单机定向恢复回执 |
| Ubuntu 22.04（jammy） | ✅ 目标支持 | 容器 CI 目标；本版无新增实机回执 |
| Ubuntu 24.04（noble） | ✅ 目标支持 | 容器 CI 目标；本版无新增实机回执 |
