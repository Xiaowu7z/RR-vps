...
- 事务式热更新：Stable 在执行前验证不可变正式 Release、精确五资产、Tag/Commit、发布所有权标记，以及同一 SHA 的 CI push 与三机稳定性 push 两类最新成功证据，Beta 使用隔离分支；同时提供持久事务日志、启动前恢复、运行快照、数据库一致性备份、配置迁移、健康门禁、失败回滚与手动恢复入口
