...
> 当前版本：**7.2.6** · [完整更新日志](CHANGELOG.md) · [GitHub Releases](https://github.com/Xiaowu7z/RR-vps/releases)

### 7.2.6：健康检查、订阅回环与隔离守护修复

修复健康检查因端口跳跃校验触发停服、本地订阅被防火墙阻断，以及隔离守护反复启动的问题。健康检查只读验证已配置的端口跳跃规则；本地订阅通过精确回环规则访问，公网阻断继续保留；升级同时处理旧守护配置和失败回滚。

洛杉矶 7.2.1 的定向恢复已由用户回报成功，服务和本机连接验收通过，原账号与订阅保留。7.2.6 将修复整合进通用安装和升级流程，包含此前本地补丁状态的兼容回归。正式发布的验证范围与实机证据边界见[7.2.6 发布记录](docs/audit/release-v726-plan.md)。

### 7.2.5：Argo 依赖下载容错

修复首次选择 Argo 时，GitHub 发布 API 返回 403/429、5xx 或网络请求失败便无法安装 Cloudflared 的问题。API 不可用时改用内置、固定版本的 Cloudflare 官方安装包，仍严格校验 SHA256、大小、DEB 结构和安装后版本；不会反复重试被拒绝的元数据请求。已安装兼容版本时直接复用，热更新事务继续禁止临时下载安装依赖。

支持 amd64/arm64；成功响应中的无效元数据、错误摘要和损坏安装包仍会拒绝。此次修复的故障依据是用户在 AWS 安装时收到的 403，未将其认定为 AWS 机房故障，也未宣称 AWS 实机重装已通过。验证范围见[下载修复记录](docs/audit/cloudflared-download-20260913.md)。

### 7.2.4：Naive 首装与防火墙隔离恢复

修复首次准备 Naive 证书后拒绝 RR 自有 systemd 保护配置、无法创建 Sing-box 主服务的问题，以及隔离恢复重建端口和端口跳跃规则时重复创建事务标记的问题。用户已回报 Debian 12、无 Nexus 的两阶段服务端恢复完成，原有身份保留；定向工具保持安装版本 7.2.3，不能将其等同于整包 7.2.4 热更新。

原三台测试 VPS 已退役。用户于 2026-09-10 确认采用[发布验证方案](docs/audit/release-v724-plan.md)：完整三发行版容器 CI，加此次单机恢复回执与发布文件的一致性核验；最终 main 提交必须通过两项门禁才能发布。本次实机证据限于 Debian 12 定向恢复，不覆盖三台实机复测或所有公网协议连通。官方 7.2.1 与 Debian 兼容版旧更新入口对新产物格式的本地兼容回归已通过，主力服务器没有被修改。
...
修复 Debian 12 / systemd 252 的日历输出格式导致 HTTPS 自动续签就绪检查误判。保留原有续签、事务和身份校验，并将实际故障输出加入发布前回归。现有 7.2.1（含 Debian 兼容修正版）可通过稳定热更新升级；旧版 Nginx 续签布局仍须先完成兼容迁移。

### 7.2.0：安全边界重构、事务恢复加固与可信发布

7.2.0 是一次覆盖订阅与管理面安全、持久热更新、跨版本回滚、加密迁移、供应链和正式发布链路的系统性工程升级。RR Nexus 实时流量统计使用的服务端核心固定到经指定上游源码提交构建的 **sing-box 1.14.0**。7.2.0–7.2.3 的 Stable 发布要求同一 `main` 提交通过 CI `push` 与三机稳定性检查 `push`；7.2.4 起第二项改为已批准方案下的恢复回执核验 `push`。Stable 继续只消费与当前 `main` 和同版本 Tag 绑定的不可变 GitHub Release；Beta 与 Stable 分离。
...
公网 IP 可信模式不是自签绕过：IP 必须全球可路由，公网 TCP/80 必须持续到达本机 ACME Webroot，证书必须由系统 CA 信任且精确匹配该 IP。任一条件不满足时，RR 不会发布公网个人订阅；可改用域名 HTTPS 或本地 SSH 隧道。

恢复会在读取目标服务状态或改写 RR 受管路径前，校验挂载拓扑、对象类型、root 所有权、写权限、硬链接及符号链接边界，并在冻结写入者、完成回滚快照和进入递归清理前重复验证。若服务越过 READY 门禁后的后期验证失败，恢复会先撤销 READY，再重新隔离 Nginx 与受管运行服务并保留恢复证据。直接安装或更新在取得统一更新锁和旧版兼容锁后，只要恢复 marker 路径存在任何对象（包括悬空链接），就会在下载发布内容和准备运行时前安全拒绝，且不会删除该证据。

`rr doctor`、加密 `.rrbak`、告警、TOTP/Passkey、历史图表、批量管理和 NaiveProxy HTTP/3 功能继续提供；`.rrbak` 与 NaiveProxy 的 7.2.0 恢复前置条件见下文，7.2.0 的安全边界、事务恢复、三发行版 CI 与当时的三机实测证据见 Changelog。

## 一键安装

目标支持系统如下。CI 覆盖三种发行版容器，实际实机范围以当轮发布报告为准。7.2.3 及此前的三机门禁属于历史发布证据；7.2.4 起采用用户已批准的三发行版容器 CI 加恢复证据核验。7.2.6 保留历史回执与原始发布包，独立核验当前修复范围及候选摘要；洛杉矶定向恢复回执不代表整包升级或公网全协议验收：

| 系统 | 支持度 | 说明 |
| --- | --- | --- |
| **Debian 12（bookworm）** | ⭐ 主推 | 容器 CI 目标；保留此前单机定向恢复回执 |
| Ubuntu 22.04（jammy） | ✅ 目标支持 | 容器 CI 目标；本版无新增实机回执 |
| Ubuntu 24.04（noble） | ✅ 目标支持 | 容器 CI 目标；洛杉矶 7.2.1 定向恢复已回报完成 |
...
3. 面板访问方式（本地 SSH 隧道 / 公网域名 / 公网 IP 可信模式）在 `14` 菜单内选择，详见下文「RR Nexus · 星枢」。
...
更新请选择主菜单 `8`；卸载请选择主菜单 `6`，再输入 `y`（大小写均可）二次确认。卸载完成后终端会再次显示项目地址和重新安装命令。
...
rr restore /path/file.rrbak  # 恢复 RR 管理数据（见下方前置条件）
rr update --check            # 只做更新预检
rr update --channel stable   # 切换 Stable；也支持 beta
rr update --rollback         # 回滚上一次已提交更新
```

`.rrbak` 只备份 RR 管理数据，不复制 Certbot 账户或续签配置。若备份启用了 NaiveProxy，目标机必须先安装当前 RR-vps，并为相同域名建立可续签的 Certbot lineage；恢复会在写入前验证证书受系统 CA 信任、域名与私钥匹配、剩余有效期不少于 7 天，以及部署钩子为当前版本。空白或未准备目标会被安全拒绝。若必须迁移到空白机，可先在源机关闭 NaiveProxy 后备份，恢复完成后再在目标机重新启用并签发证书。

仅复制 `fullchain.pem` 与 `privkey.pem` 不符合要求：Certbot `live` 中的 `cert`、`chain`、`fullchain`、`privkey` 四个链接必须指向同一 `archive` 代次，续签配置必须绑定生产 Let's Encrypt ACME 账户和 RR 的 ACME Webroot，相关路径还须通过严格权限检查。

仅当备份未启用 NaiveProxy 时，才支持直接恢复到空白目标；此时订阅默认仅监听回环地址。已有目标机的独立 HTTPS 订阅访问面会保留，但其可信证书和当前部署钩子也必须通过恢复前检查。活动 UFW 只接受可证明与 RR 端口互不重叠的简单用户规则；同端口规则或无法安全证明顺序独立的复杂规则会在任何防火墙写入前拒绝恢复，请先备份并人工整理冲突规则。
...
- TUIC v5
- AnyTLS
- NaiveProxy（原生 naive inbound，可选 HTTP/2、HTTP/3/QUIC 或双模式，支持 QUIC 拥塞控制与 Let's Encrypt 真证书；安装 RR Nexus 后，承担流量统计的服务端内核固定为 sing-box 1.14.0；Clash 原生不支持自动跳过）
- 每种协议独立开关、独立端口修改
- IPv4 / IPv6 入口与出口独立选择
- 可信域名 HTTPS 订阅，以及无需域名、使用 Let's Encrypt 短期证书的全球公网 IPv4/IPv6 Nexus 个人订阅；NAT/LXD 必须让公网 TCP/80 和所选 HTTPS 端口精确映射到本机
- Sing-box 配置校验、健康检查和异常自动重启
- 防火墙端口变更采用 UFW / IPv4 / IPv6 目标规则事务：首个写入前先持久建立跨重启门禁并停用受管入口，全部参与后端的 live 与持久状态验证通过后才解除；部分写入、保存或补偿无法证明时保留隔离证据，只有完整修复并复核后才恢复入口
- 菜单、CLI、后台同步、健康修复和证书部署等写入口统一进入 root 专用事务锁域；Naive/Sing-box 与 Nexus 公网证书对在原子发布、有效 systemd 门禁和实际服务证书证明通过前保持关闭，失败可由持久 pending 证据幂等补偿
...
- 事务式热更新：Stable 在执行前验证不可变正式 Release、精确五资产、Tag/Commit、发布所有权标记，以及同一 SHA 的 CI push 与发布验证工作流 push 两类最新成功证据（各版本验证范围见发布报告），Beta 使用隔离分支；同时提供持久事务日志、启动前恢复、运行快照、数据库一致性备份、配置迁移、健康门禁、失败回滚与手动恢复入口
- `rr doctor` 系统 / DNS / 时间 / 公网网络 / 核心 / 端口 / 防火墙 / 证书 / 面板 / 订阅 / 数据库 / 磁盘 / 更新源体检，支持安全修复与自动脱敏报告
- 密码加密的 `.rrbak` RR 管理数据备份、跨 VPS 迁移与失败时自动尝试原机回滚；它不是整机备份，只恢复 RR 管理范围，不覆盖其他应用服务
...
| NaiveProxy H2 / H3 | ✅（原生 naive；RR Nexus 统计核心固定 1.14.0） | ❌ | 视客户端内核 | 视版本 | 视版本 |

使用提示：锁屏/挂起后需要快速恢复网络通道的场景，建议配合「主动心跳模式」（主菜单 9 → 11）使用，并优先选择 sing-box 内核 1.14+ 的客户端。

## 订阅访问安全

公网明文 HTTP 订阅已移除。Nexus 面板和独立订阅服务分别采用以下安全状态；`SUB_ACCESS_MODE` 只描述独立订阅服务，不代表 Nexus 面板模式。

| 订阅状态 | 地址 | 安全要求 |
|---|---|---|
| Nexus 可信域名 | `https://可信域名[:面板端口]/sub/...` | 域名精确匹配有效、受系统 CA 信任的证书 |
| Nexus 可信公网 IP | IPv4：`https://公网IPv4:面板端口/sub/...`；IPv6：`https://[公网IPv6]:面板端口/sub/...` | 必须选择全球可路由的公网 IP；Let's Encrypt `shortlived` IP 证书已完成原子发布，SAN/私钥/系统 CA 链及剩余有效期校验全部通过；公网 TCP/80 必须持续到达 ACME Webroot |
...
公网 IP 模式使用 Let's Encrypt 短期证书（当前约 6–7 天有效），RR 每 12 小时运行一次持久续签任务并保留 HTTP-01 Webroot。端口 80 被阻断、IP 不属于本机、证书链不受系统 CA 信任、证书过期或证书对处于 pending 状态时，面板不会回退到自签订阅，也不会生成可公开分享的个人订阅地址。部分过旧客户端可能尚不支持受信 IP 证书，应升级客户端或使用域名/SSH 隧道。

没有满足上述条件的可信证书时，请在运行客户端的设备上建立订阅端口转发。假设脚本显示订阅端口为 `39291`、示例服务器 IP 为 `203.0.113.10`：
...
| 公网 IP 可信模式 | 全球可路由的服务器 IPv4/IPv6 + Let's Encrypt 短期 IP 证书 | 无域名但需要公网面板和个人订阅 | 必须开放并保持公网 TCP/80；证书精确匹配 IP 且由系统 CA 信任，不提供自签回退 |

公网面板的登录和页面正文只通过 HTTPS 提供。域名模式和公网 IP 可信模式的 80 端口仅用于 ACME HTTP-01 challenge（域名模式签发后还可跳转到 HTTPS），其他明文请求不返回面板或订阅正文。安装向导会要求管理员账号和强密码，并启用 Argon2id 哈希、应用与 Nginx 双层登录限流、CSRF 防护、12 小时会话、8 个一次性恢复码和审计日志。
...
公网域名面板与通过全部证书门禁的公网 IP 面板，都会在同一可信 HTTPS 入口提供 `/sub/...` 个人订阅；IPv6 地址按 URI 规范使用方括号。这些请求关闭 Nginx 访问日志，后端错误日志也不记录 URL，因此 token 不会进入 request-line 日志。历史遗留的自签 IP 证书只能视为面板兼容状态，绝不会解锁公网订阅或远程钥匙；若可信 IP 证书无法签发或续签，必须改用可信订阅域名或本地 SSH 隧道。本地面板只返回 `http://127.0.0.1:订阅端口/nexus/<随机令牌>.txt` 一类回环地址，必须同时转发订阅端口。
...
RR Nexus 安装时会下载由项目 GitHub Actions 从官方 sing-box 1.14.0 固定源码提交构建、额外启用 `with_v2ray_api` 的统计内核，并校验版本、源码修订、构建标签与 SHA256。面板每 5 秒读取以设备 ID 命名的用户计数器，累计上传和下载，在达到额度时撤销该设备。个人设备统计属于应用层计量，Sing-box 异常退出或重载瞬间仍可能丢失少量尚未采集的数据。
...
| `86-nexus-ip-acme.sh` | 公网 IPv4/IPv6 短期证书、续签、崩溃恢复与安全卸载 |
