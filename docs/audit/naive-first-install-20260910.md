# 7.2.4：Naive 首次安装中断修复

状态：已收到用户的 Debian 12 服务端定向恢复成功回执；用户于 2026-09-10 确认新的验证范围并授权发布 7.2.4。实机证据仍限于原安装版本 7.2.3 上的定向恢复，尚未收到整包 7.2.4 热更新、全新安装或客户端公网协议连通测试回执。实际发布完成以远端门禁和 Release 核验结果为准。
...
聚焦回归使用临时目录中的真实文件、权限和链接，systemctl 由 fixture 提供；不能算实机 systemd 或公网协议连接测试。用户随后提供了故障机现有状态上的完整恢复成功回执，见下文“两阶段恢复完成”。这不是空白服务器全新安装、三个系统复测或所有协议端到端验收。

已完成的本地检查：相关 Shell 语法与确定性发布包核验通过；22 项所有权回归通过，其中 foreign-owner 一项因容器不能改变 UID，使用了明确报告的元数据注入。恢复工具的成功退出、前期失败、后期失败三个 EXIT 生命周期 fixture 通过。这些本地检查没有执行恢复工具的真实写操作。以下排查记录保留各次失败发生时的判断；后续实机结果及发布授权见末尾记录。
...
旧三台云测试机已由用户删除，不得再次连接。当时的候选修复仅推独立分支，没有改变 main、Latest 或正式发布校验。后续已将退役主机工作流归档，并制定经用户确认的新发布验证方案；详情见 `release-v724-plan.md`。未运行的测试不计为通过。
...
定向工具现在额外下载并校验该 system 模块，仅替换日志目录下的私有模块副本；已安装的不可变 7.2.3 文件继续前后核对，不直接覆盖。模块 30 仍来自此前固定提交。新版工具 SHA256 为 `5d3ee4294126212aac54b532ca5165cb0ee8a1757483f69aa88d4cf0cd24ba0e`。当时仍需用户执行后取得 `FIREWALL_RECOVERY_COMPLETE`、`REPAIR_COMPLETE` 并反馈实际使用结果，不能将本地复现计为故障机恢复成功。下一节记录了随后收到的服务端恢复回执。
...
现成的 `tests/test-upgrade-721-release.sh` 已使用当前真实候选产物完成本地兼容回归：官方 7.2.1 及 Debian 兼容版两份旧清单均通过旧 guard 的更新发现与 bootstrap 获取；main 移动、缺少 push 证据、非 immutable Release 和 bootstrap 篡改均拒绝。测试中的 GitHub 响应为 fixture，不执行安装器或访问主力服务器。用户已于 2026-09-10 确认采用完整三发行版容器 CI 加本次单机回执核验的方案，并授权发布 7.2.4。最终 main 提交仍须通过两项发布门禁；具体方案见 `release-v724-plan.md`。
